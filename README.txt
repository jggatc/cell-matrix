...
Instructions:
Cell Matrix runs with Python and the Pygame library. Input of Game of Life patterns can be done from the interface panel by the controls LOAD from a customized pattern.txt file and GET from the system clipboard. The app is run with the command 'python run_app.py'. Alternatively, the app can run in the Java environment (http://www.java.com/getjava/) using Jython 2.2.1+ and the PyJ2D library with the command 'jython cell.py' or 'java -jar jython.jar cell.py'. The app can also run in the JavaScript environment using Pyjs (http://www.pyjs.org/) compilation and the Pyjsdl library with the command '[pyjs_path]/bin/pyjsbuild -O cell.py --dynamic-link -o output' and copying data folder to pyjsbuild output.
